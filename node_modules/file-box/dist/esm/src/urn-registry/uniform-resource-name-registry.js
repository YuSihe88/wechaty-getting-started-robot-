...
class UniformResourceNameRegistry {
    constructor(options = {}) {
        ...
        this.storeDir = options.storeDir || path.join(os.tmpdir(), 'uniform-resource-name-registry.' + String(process.pid));
    }
    async init() {
    ...
    uuidFile(uuid) {
        return path.join(this.storeDir, uuid);
    }
    /**
    ...
    unlinkFactory(uuid) {
        ...
        return async () => {
            try {
                ...
                log.warn('UniformResourceNameRegistry', 'unlinkFactory() unlink() rejection:', e.message);
            }
        };
    }
    destroy() {
