...
class SocketClient {
    ...
    send(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                brolog_1.log.silly(this.LOGPRE, `[tid:${this.traceId}] socket #${this.retryStrategy.retryCount} retry send, after delay: ${delay}ms, addr:\"${this.host}:${this.port}\" data:${ByteUtils_1.bytesToHexString(data, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
    ...
    _sendImpl(sendData) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._socket) {
                brolog_1.log.warn(this.LOGPRE, "can not send again while socket is working");
                return;
            }
            ...
            const sendDataBlock = (socket, dataQueue) => {
                ...
                brolog_1.log.silly(this.LOGPRE, `socket send:${ByteUtils_1.bytesToHexString(block, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
            ...
            return new Promise((resolve, reject) => {
                const onSocketFinish = (error, retryOnError = true) => {
                    ...
                    if (error) {
                        ...
                        brolog_1.log.silly(this.LOGPRE, `socket on error: ${error}, retryOnError:${retryOnError}`);
                    ...
                    brolog_1.log.silly(this.LOGPRE, `destroy socket`);
                    this._socket.destroy();
                    this._socket = undefined;
                };
                const startDate = new Date();
                brolog_1.log.silly(this.LOGPRE, `socket start connect: ${this.host}:${this.port}`);
                ...
                socket.connect({
                    ...
                    brolog_1.log.silly(this.LOGPRE, `socket connect success, cost: ${endDate.getTime() - startDate.getTime()}ms`);
                    this._callbackExecutor.execute(() => __awaiter(this, void 0, void 0, function* () {
                        var _a, _b;
                ...
                socket.on("data", (data) => {
                    brolog_1.log.silly(this.LOGPRE, `socket recv:${ByteUtils_1.bytesToHexString(data, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
                    this._callbackExecutor.execute(() => __awaiter(this, void 0, void 0, function* () {
                        try {
                            const finished = yield this._callback.onReceive(data);
                            brolog_1.log.silly(this.LOGPRE, `process data, finished:${finished}`);
                            if (finished) {
