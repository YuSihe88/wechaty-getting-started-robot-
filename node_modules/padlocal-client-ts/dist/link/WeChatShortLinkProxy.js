...
class WeChatShortLinkProxy {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.retryStrategy.canRetry()) {
                    const message = `[tid:${this.traceId}] Fail to request short link for path:${path}, data: ${ByteUtils_1.bytesToHexString(data, ByteUtils_1.MAX_LOG_BYTES_LEN)}, after max retry:${this.retryStrategy.retryCount}`;
                    throw new erros_1.IOError(e, message);
                }
                const delay = this.retryStrategy.nextRetryDelay();
                brolog_1.log.silly(LOGPRE, `[tid:${this.traceId}] short link #${this.retryStrategy.retryCount} retry request, after delay: ${delay}ms, path: ${path} data: ${ByteUtils_1.bytesToHexString(data, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
    ...
    _sendImpl(path, data) {
        return __awaiter(this, void 0, void 0, function* () {
            brolog_1.log.silly(LOGPRE, `[tid:${this.traceId}] short link send, ${this.host}:${this.port}${path}, request: ${ByteUtils_1.bytesToHexString(data, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
            return new Promise((resolve, reject) => {
                ...
                const req = http_1.default.request(`http://${this.host}:${this.port}${path}`, {
                    ...
                    res.on("end", () => {
                        brolog_1.log.silly(LOGPRE, `[tid:${this.traceId}] short link receive, response: ${ByteUtils_1.bytesToHexString(responseBuffer, ByteUtils_1.MAX_LOG_BYTES_LEN)}`);
