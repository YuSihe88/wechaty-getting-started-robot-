...
exports.mockerifyRoomMock = exports.RoomMock = void 0;
const wechaty_puppet_1 = require("wechaty-puppet");
const room_events_js_1 = require("../events/room-events.js");
const POOL = Symbol('pool');
class RoomMock extends room_events_js_1.RoomEventEmitter {
    constructor(payload) {
        super();
        this.payload = payload;
        wechaty_puppet_1.log.silly('MockRoom', 'constructor(%s)', JSON.stringify(payload));
    ...
    static get pool() {
        if (!this[POOL]) {
            wechaty_puppet_1.log.verbose('MockRoom', 'get pool() init pool');
            this[POOL] = new Map();
        }
        if (this === RoomMock) {
    ...
    static create(payload) {
        wechaty_puppet_1.log.verbose('MockRoom', 'static create(%s)', JSON.stringify(payload));
    ...
    topic(text) {
        wechaty_puppet_1.log.verbose('MockRoom', 'topic(%s)', text);
        const that = this;
        return { by };
        function by(contact) {
            wechaty_puppet_1.log.verbose('MockRoom', 'topic(%s).by(%s)', text, contact?.id || '');
            if (!contact) {
    ...
    add(...inviteeList) {
        wechaty_puppet_1.log.verbose('MockRoom', 'add(%s)', inviteeList.map(i => i.id).join(','));
        const that = this;
        return { by };
        function by(inviter) {
            wechaty_puppet_1.log.verbose('MockRoom', 'add(%s).by(%s)', inviteeList.map(i => i.id).join(','), inviter?.id || '');
            if (!inviter) {
    ...
    remove(...removeeList) {
        wechaty_puppet_1.log.verbose('MockRoom', 'remove(%s)', removeeList.map(i => i.id).join(','));
        const that = this;
        return { by };
        function by(remover) {
            wechaty_puppet_1.log.verbose('MockRoom', 'remove(%s).by(%s)', removeeList.map(i => i.id).join(','), remover?.id || '');
            if (!remover) {
...
exports.mockerifyRoomMock = mockerifyRoomMock;
