...
const conversation_id_file_box_js_1 = require("./conversation-id-file-box.js");
(0, tstest_1.test)('unpackConversationIdFileBoxArgsFromPb()', async (t) => {
    ...
    const req1 = new wechaty_grpc_1.puppet.MessageSendFileStreamRequest();
    req1.setConversationId(CONVERSATION_ID);
    stream.write(req1);
    const req2 = new wechaty_grpc_1.puppet.MessageSendFileStreamRequest();
    ...
    fileBoxStream.on('data', chunk => {
        ...
        const req3 = new wechaty_grpc_1.puppet.MessageSendFileStreamRequest();
        req3.setFileBoxChunk(fileBoxChunk);
        stream.write(req3);
    });
    fileBoxStream.on('end', () => stream.end());
    const args = await (0, conversation_id_file_box_js_1.unpackConversationIdFileBoxArgsFromPb)(stream);
...
(0, tstest_1.test)('packConversationIdFileBoxToPb()', async (t) => {
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const CONVERSATION_ID = 'conv_id';
    const fileBox = wechaty_puppet_1.FileBox.fromBuffer(Buffer.from(FILE_BOX_DATA), FILE_BOX_NAME);
    const stream = await (0, conversation_id_file_box_js_1.packConversationIdFileBoxToPb)(wechaty_grpc_1.puppet.MessageSendFileStreamRequest)(CONVERSATION_ID, fileBox);
    const data1 = await (0, next_data_js_1.nextData)(stream);
    t.equal(data1.getConversationId(), CONVERSATION_ID, 'match conversation id');
    const data2 = await (0, next_data_js_1.nextData)(stream);
...
(0, tstest_1.test)('unpackConversationIdFileBoxArgsFromPb() <-> packConversationIdFileBoxToPb()', async (t) => {
    const FILE_BOX_DATA = 'test';
    const FILE_BOX_NAME = 'test.dat';
    const CONVERSATION_ID = 'conv_id';
    const fileBox = wechaty_puppet_1.FileBox.fromBuffer(Buffer.from(FILE_BOX_DATA), FILE_BOX_NAME);
