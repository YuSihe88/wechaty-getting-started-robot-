...
class EventStreamManager {
    constructor(puppet) {
        ...
        wechaty_puppet_1.log.verbose('EventStreamManager', 'constructor(%s)', puppet);
    }
    busy() {
        return !!this.eventStream;
    }
    start(stream) {
        wechaty_puppet_1.log.verbose('EventStreamManager', 'start(stream)');
        if (this.eventStream) {
        ...
        if (this.puppet.logonoff()) {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'start() puppet is logged in, emit a login event for downstream');
            const payload = {
    ...
    stop() {
        wechaty_puppet_1.log.verbose('EventStreamManager', 'stop()');
        if (!this.eventStream) {
    ...
    obj) {
        wechaty_puppet_1.log.verbose('EventStreamManager', 'grpcEmit(%s[%s], %s)', event_type_rev_js_1.EventTypeRev[type], type, JSON.stringify(obj));
        ...
        if (this.eventStream) {
            ...
            /**
              ...
              */
            wechaty_puppet_1.log.warn('EventStreamManager', 'grpcEmit(%s, %s) this.eventStream is undefined.', type, JSON.stringify(obj));
        }
    }
    connectPuppetEventToStreamingCall() {
        wechaty_puppet_1.log.verbose('EventStreamManager', 'connectPuppetEventToStreamingCall() for %s', this.puppet);
        const offCallbackList = [];
        const offAll = () => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'connectPuppetEventToStreamingCall() offAll() %s callbacks', offCallbackList.length);
            offCallbackList.forEach(off => off());
            this.puppetListening = false;
        };
        const eventNameList = Object.keys(wechaty_puppet_1.PUPPET_EVENT_DICT);
        for (const eventName of eventNameList) {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'connectPuppetEventToStreamingCall() this.puppet.on(%s) (listenerCount:%s) registering...', eventName, this.puppet.listenerCount(eventName));
            switch (eventName) {
                case 'dong': {
    ...
    onStreamingCallEnd(removePuppetListeners) {
        wechaty_puppet_1.log.verbose('EventStreamManager', 'onStreamingCallEnd(callback)');
        if (!this.eventStream) {
            throw new Error('no this.eventStream found');
        }
        this.eventStream.on('cancelled', () => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(cancelled) fired with arguments: %s', JSON.stringify(arguments));
            ...
            if (this.eventStream) {
                ...
                wechaty_puppet_1.log.warn('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(cancelled) eventStream is undefined');
            }
        });
        this.eventStream.on('error', err => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(error) fired: %s', err);
            if (this.puppetListening) {
                removePuppetListeners();
            }
            if (this.eventStream) {
                ...
                wechaty_puppet_1.log.warn('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(error) eventStream is undefined');
            }
        });
        this.eventStream.on('finish', () => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(finish) fired');
            if (this.puppetListening) {
                removePuppetListeners();
            }
            if (this.eventStream) {
                ...
                wechaty_puppet_1.log.warn('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(finish) eventStream is undefined');
            }
        });
        this.eventStream.on('end', () => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(end) fired');
            if (this.puppetListening) {
                removePuppetListeners();
            }
            if (this.eventStream) {
                ...
                wechaty_puppet_1.log.warn('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(end) eventStream is undefined');
            }
        });
        this.eventStream.on('close', () => {
            wechaty_puppet_1.log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(close) fired');
            if (this.puppetListening) {
                removePuppetListeners();
            }
            if (this.eventStream) {
                ...
                wechaty_puppet_1.log.warn('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(close) eventStream is undefined');
            }
        });
    }
}
