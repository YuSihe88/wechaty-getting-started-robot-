...
class FlashStore {
     ...
     */
    async set(key, value) {
        config_js_1.log.verbose('FlashStore', 'set(%s, %s)', key, typeof value);
        config_js_1.log.silly('FlashStore', 'set(%s, %s)', key, JSON.stringify(value));
     ...
     */
    async get(key) {
        config_js_1.log.verbose('FlashStore', 'get(%s)', key);
        try {
            ...
            if (e instanceof level_errors_1.default.NotFoundError) {
                return undefined;
            }
            throw e;
        }
    ...
    // }
    async delete(key) {
        config_js_1.log.verbose('FlashStore', 'delete(%s)', key);
     ...
     */
    async *keys(options = {}) {
        config_js_1.log.verbose('FlashStore', 'keys()');
    ...
    async *values(options = {}) {
        config_js_1.log.verbose('FlashStore', 'values()');
        // options = Object.assign(options, {
        //   keys   : false,
        //   values : true,
        // })
    ...
    // }
    get size() {
        config_js_1.log.verbose('FlashStore', 'size()');
    ...
    [Symbol.iterator]() {
        config_js_1.log.verbose('FlashStore', '[Symbol.iterator]()');
        /**
    ...
    async *entries(options) {
        config_js_1.log.verbose('FlashStore', '*entries(%s)', JSON.stringify(options));
    ...
    async *[Symbol.asyncIterator]() {
        config_js_1.log.verbose('FlashStore', '*[Symbol.asyncIterator]()');
        yield* this.entries();
    }
    /**
     * @private
    ...
    async forEach(callbackfn, thisArg) {
        config_js_1.log.verbose('FlashStore', 'forEach()');
        for await (const [key, value] of this) {
            callbackfn.call(thisArg, value, key, this);
        }
    }
    async close() {
        config_js_1.log.verbose('FlashStore', 'close()');
     ...
     */
    async destroy() {
        config_js_1.log.verbose('FlashStore', 'destroy()');
        await this.levelDb.close();
        await new Promise(resolve => (0, rimraf_1.default)(this.workdir, resolve));
    }
}
exports.FlashStore = FlashStore;
FlashStore.VERSION = config_js_1.VERSION;
