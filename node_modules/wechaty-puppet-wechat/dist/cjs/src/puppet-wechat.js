...
class PuppetWeChat extends wechaty_puppet_1.Puppet {
    constructor(options = {}) {
        ...
        this.bridge = new bridge_js_1.Bridge({
            ...
            head: typeof options.head === 'boolean' ? options['head'] : (0, config_js_1.envHead)(),
            launchOptions: options.launchOptions,
            memory: this.memory,
            stealthless: typeof options.stealthless === 'boolean' ? options.stealthless : (0, config_js_1.envStealthless)(),
        });
    ...
    async start() {
        wechaty_puppet_1.log.verbose('PuppetWeChat', `start() with ${this.memory.name}`);
        if (this.state.on()) {
            wechaty_puppet_1.log.warn('PuppetWeChat', 'start() is called on a ON puppet. await ready(on) and return.');
        ...
        try {
            ...
            wechaty_puppet_1.log.verbose('PuppetWeChat', 'initBridge() done');
            /**
            ...
            throttleQueue.subscribe(async (data) => {
                wechaty_puppet_1.log.verbose('PuppetWeChat', 'start() throttleQueue.subscribe() new item: %s', data);
                await this.saveCookie();
            });
            wechaty_puppet_1.log.verbose('PuppetWeChat', 'start() done');
            // this.emit('start')
            return;
        }
        catch (e) {
            wechaty_puppet_1.log.error('PuppetWeChat', 'start() exception: %s', e);
            // this.state.off(true)
            this.emit('error', {
                data: e.message,
            });
    ...
    async stop() {
        wechaty_puppet_1.log.verbose('PuppetWeChat', 'stop()');
        if (this.state.off()) {
            wechaty_puppet_1.log.warn('PuppetWeChat', 'stop() is called on a OFF puppet. await ready(off) and return.');
            await this.state.ready('off');
            return;
        }
        this.state.off('pending');
        wechaty_puppet_1.log.verbose('PuppetWeChat', 'stop() make watchdog sleep before do stop');
        await super.stop();
        /**
        ...
        try {
            ...
            wechaty_puppet_1.log.error('PuppetWeChat', 'this.bridge.quit() exception: %s', e.message);
            throw e;
        }
        finally {
            this.state.off(true);
    ...
    async initBridge() {
        ...
        this.bridge.on('login', event_js_1.Event.onLogin.bind(this));
        this.bridge.on('logout', event_js_1.Event.onLogout.bind(this));
        this.bridge.on('message', event_js_1.Event.onMessage.bind(this));
        this.bridge.on('scan', event_js_1.Event.onScan.bind(this));
        this.bridge.on('unload', event_js_1.Event.onUnload.bind(this));
        try {
            await this.bridge.start();
        }
        catch (e) {
    ...
    unref() {
        wechaty_puppet_1.log.verbose('PuppetWeChat', 'unref ()');
        super.unref();
    ...
    async messageRecall(messageId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(messageId);
    }
    ...
    async messageSendUrl(conversationId, urlLinkPayload) {
        (0, wechaty_puppet_1.throwUnsupportedError)(conversationId, urlLinkPayload);
    }
    async messageSendMiniProgram(conversationId, miniProgramPayload) {
        ...
        (0, wechaty_puppet_1.throwUnsupportedError)(conversationId, miniProgramPayload);
    }
    /**
     ...
     */
    async logout() {
        wechaty_puppet_1.log.verbose('PuppetWeChat', 'logout()');
    ...
    async contactSelfName(name) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(name);
    }
    async contactSelfSignature(signature) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(signature);
    }
    /**
     *
     * Contact
    ...
    async contactRawPayloadParser(rawPayload) {
        ...
        return {
            ...
            type: (!!rawPayload.UserName && !rawPayload.UserName.startsWith('@@') && !!(rawPayload.VerifyFlag & 8))
                ...
                : wechaty_puppet_1.ContactType.Individual,
            ...
            while (ttl-- /* && currNum !== prevNum */) {
                ...
                if (rawPayload) {
                    ...
                    if (prevLength === currLength) {
                        wechaty_puppet_1.log.silly('PuppetWeChat', 'roomPayload() puppet.getContact(%s) done at ttl:%d with length:%d', this.id, ttl, currLength);
                        return rawPayload;
                    }
    ...
    async roomQRCode(roomId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(roomId);
    }
    ...
    async roomInvitationAccept(roomInvitationId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(roomInvitationId);
    }
    async roomInvitationRawPayload(roomInvitationId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(roomInvitationId);
    }
    async roomInvitationRawPayloadParser(rawPayload) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(rawPayload);
    }
    /**
     *
     * Friendship
    ...
    async friendshipSearchPhone(phone) {
        throw (0, wechaty_puppet_1.throwUnsupportedError)(phone);
    }
    async friendshipSearchWeixin(weixin) {
        throw (0, wechaty_puppet_1.throwUnsupportedError)(weixin);
    }
    ...
    async uploadMedia(file, toUserName) {
        ...
        switch (ext) {
            ...
            case '.png':
            case '.gif':
                mediatype = web_schemas_js_1.WebMediaType.Image;
                break;
            case '.mp4':
                mediatype = web_schemas_js_1.WebMediaType.Video;
                break;
            default:
                mediatype = web_schemas_js_1.WebMediaType.Attachment;
        }
        const buffer = await new Promise((resolve, reject) => {
            const bl = new bl_1.default((err, data) => {
                if (err)
                    reject(err);
                else
                    resolve(data);
        ...
        if (mediatype === web_schemas_js_1.WebMediaType.Video && buffer.length > MAX_VIDEO_SIZE) {
    ...
    async tagContactAdd(tagId, contactId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(tagId, contactId);
    }
    async tagContactRemove(tagId, contactId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(tagId, contactId);
    }
    async tagContactDelete(tagId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(tagId);
    }
    async tagContactList(contactId) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(contactId);
    }
    async contactCorporationRemark(contactId, corporationRemark) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(contactId, corporationRemark);
    }
    async contactDescription(contactId, description) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(contactId, description);
    }
    async contactPhone(contactId, phoneList) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(contactId, phoneList);
    }
    conversationReadMark(conversationId, hasRead = true) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(conversationId, hasRead);
    }
}
exports.PuppetWeChat = PuppetWeChat;
PuppetWeChat.VERSION = config_js_1.VERSION;
