...
import { ContactType, FriendshipType, FileBox, MessageType, Puppet, throwUnsupportedError, log, } from 'wechaty-puppet';
import { envStealthless, envHead, MEMORY_SLOT, qrCodeForChatie, VERSION, } from './config.js';
...
import { WebAppMsgType, WebMediaType, WebMessageType, } from './web-schemas.js';
...
export class PuppetWeChat extends Puppet {
            ...
            while (ttl-- /* && currNum !== prevNum */) {
                ...
                if (rawPayload) {
                    ...
                    if (prevLength === currLength) {
                        log.silly('PuppetWeChat', 'roomPayload() puppet.getContact(%s) done at ttl:%d with length:%d', this.id, ttl, currLength);
                        return rawPayload;
                    }
    ...
    async uploadMedia(file, toUserName) {
        ...
        const buffer = await new Promise((resolve, reject) => {
            const bl = new BufferList((err, data) => {
                if (err)
                    reject(err);
                else
                    resolve(data);
