...
class PuppetWechat4u extends wechaty_puppet_1.Puppet {
    constructor(options = {}) {
        super(options);
        ...
        this.cacheMessageRawPayload = new lru_cache_1.default(lruOptions);
    }
    async start() {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', `start() with ${this.memory.name}`);
        this.state.on('pending');
        if (this.wechat4u) {
            wechaty_puppet_1.log.warn('PuppetWechat4u', 'start() wechat4u exist, will be overwrited');
        }
    ...
    initHookEvents(wechat4u) {
        ...
        this.wechat4u.on('uuid', (uuid) => {
            ...
            this.emit('scan', { qrcode: this.scanQrCode, status: wechaty_puppet_1.ScanStatus.Waiting });
        });
        /**
        ...
        wechat4u.on('user-avatar', (avatarDataUrl) => {
            this.emit('scan', {
                ...
                status: wechaty_puppet_1.ScanStatus.Scanned,
            });
        });
        /**
         * 登录成功事件
    ...
    async stop() {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', 'stop()');
        if (this.state.off()) {
            wechaty_puppet_1.log.warn('PuppetWechat4u', 'quit() is called on a OFF puppet. await ready(off) and return.');
    ...
    async logout() {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', 'logout()');
        if (!this.id) {
    ...
    unref() {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', 'unref()');
        super.unref();
        // TODO: unref wechat4u
    }
    /**
    ...
    async contactAvatar(contactId, file) {
         ...
         * 如何获取联系人头像
         */
        return wechaty_puppet_1.FileBox.fromStream(res.data, `wechaty-contact-avatar-${name}.jpg`);
    }
    ...
    async messageImage(messageId, imageType) {
        wechaty_puppet_1.log.verbose('PuppetWechat4u', 'messageImage(%s, %s[%s])', messageId, imageType, wechaty_puppet_1.ImageType[imageType]);
        return (0, wechaty_puppet_1.throwUnsupportedError)();
    }
    async messageFile(id) {
        ...
        switch (rawPayload.MsgType) {
            ...
            case this.wechat4u.CONF.MSGTYPE_IMAGE:
                ...
                return wechaty_puppet_1.FileBox.fromStream((await this.wechat4u.getMsgImg(rawPayload.MsgId)).data, filename);
            case this.wechat4u.CONF.MSGTYPE_VOICE:
                /**
                 * 语音消息
                 */
                // console.log('语音消息，保存到本地')
                return wechaty_puppet_1.FileBox.fromStream((await this.wechat4u.getVoice(rawPayload.MsgId)).data, filename);
            ...
            case this.wechat4u.CONF.MSGTYPE_MICROVIDEO:
                ...
                return wechaty_puppet_1.FileBox.fromStream((await this.wechat4u.getVideo(rawPayload.MsgId)).data, filename);
            case this.wechat4u.CONF.MSGTYPE_APP:
                if (rawPayload.AppMsgType === 6) {
                    ...
                    return wechaty_puppet_1.FileBox.fromStream((await this.wechat4u.getDoc(rawPayload.FromUserName, rawPayload.MediaId, rawPayload.FileName)).data, filename);
                }
                break;
            default:
                break;
    ...
    async messageSendContact(conversationId, contactId) {
        ...
        (0, wechaty_puppet_1.throwUnsupportedError)();
    }
    ...
    async roomAnnounce(roomId, text) {
        return (0, wechaty_puppet_1.throwUnsupportedError)(roomId, text);
    }
...
PuppetWechat4u.VERSION = config_js_1.VERSION;
