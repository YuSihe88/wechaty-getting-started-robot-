...
class PuppetWhatsapp extends wechaty_puppet_1.Puppet {
    constructor(options = {}) {
        super(options);
        this.options = options;
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'constructor()');
        this.messageStore = {};
        this.contactStore = {};
    }
    async start() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'start()');
        if (this.state.on()) {
            wechaty_puppet_1.log.warn('PuppetWhatsApp', 'start() is called on a ON puppet. await ready(on) and return.');
            await this.state.ready('on');
            return;
        }
        this.state.on('pending');
        const session = await this.memory.get(config_js_1.MEMORY_SLOT);
        const whatsapp = await (0, whatsapp_js_1.getWhatsApp)(session);
        ...
        whatsapp
            .initialize()
            .then(() => wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'start() whatsapp.initialize() done'))
            .catch(e => {
            if (this.state.on()) {
                console.error(e);
                wechaty_puppet_1.log.error('PuppetWhatsApp', 'start() whatsapp.initialize() rejection: %s', e);
            }
            else {
                // Puppet is stoping...
                wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'start() whatsapp.initialize() rejected on a stopped puppet.');
            }
        });
        await super.start();
        /**
    ...
    async stop() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'stop()');
        if (this.state.off()) {
            wechaty_puppet_1.log.warn('PuppetWhatsApp', 'stop() is called on a OFF puppet. await ready(off) and return.');
            await this.state.ready('off');
            return;
        }
        if (!this.whatsapp) {
            wechaty_puppet_1.log.error('PuppetWhatsApp', 'stop() this.whatsapp is undefined!');
            return;
        }
    ...
    initWhatsAppEvents(whatsapp) {
        wechaty_puppet_1.log.verbose('PuppetwhatsApp', 'initWhatsAppEvents()');
        whatsapp.on('authenticated', async (session) => {
            try {
                // save session file
                await this.memory.set(config_js_1.MEMORY_SLOT, session);
                await this.memory.save();
            }
            catch (e) {
                console.error(e);
                wechaty_puppet_1.log.error('PuppetWhatsApp', 'getClient() whatsapp.on(authenticated) rejection: %s', e);
            }
        });
        ...
        whatsapp.on('qr', (qr) => {
            ...
            this.emit('scan', { qrcode: qr, status: wechaty_puppet_1.ScanStatus.Waiting });
        });
    }
    ...
    // }
    ding(data) {
        wechaty_puppet_1.log.silly('PuppetWhatsApp', 'ding(%s)', data || '');
        setTimeout(() => this.emit('dong', { data: data || '' }), 1000);
    }
    unref() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'unref()');
        super.unref();
        if (this.loopTimer) {
    ...
    async contactSelfQRCode() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactSelfQRCode()');
        return config_js_1.CHATIE_OFFICIAL_ACCOUNT_QRCODE;
    }
    async contactSelfName(name) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactSelfName(%s)', name);
    }
    async contactSelfSignature(signature) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactSelfSignature(%s)', signature);
    }
    async contactAlias(contactId, alias) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactAlias(%s, %s)', contactId, alias);
        if (typeof alias === 'undefined') {
            return 'mock alias';
        }
    }
    async contactPhone(contactId, phoneList) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactPhone(%s, %s)', contactId, phoneList);
        if (typeof phoneList === 'undefined') {
            return [];
        }
    }
    async contactCorporationRemark(contactId, corporationRemark) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactCorporationRemark(%s, %s)', contactId, corporationRemark);
    }
    async contactDescription(contactId, description) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactDescription(%s, %s)', contactId, description);
    }
    async contactList() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactList()');
        return [];
    }
    async contactQRCode(contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactQRCode(%s)', contactId);
    ...
    async contactAvatar(contactId, file) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactAvatar(%s)', contactId);
        /**
         * 1. set
         */
        if (file) {
    ...
    async contactRawPayload(id) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'contactRawPayload(%s)', id);
    ...
    async conversationReadMark(conversationId, hasRead) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'conversationReadMark(%s, %s)', conversationId, hasRead);
        return (0, wechaty_puppet_1.throwUnsupportedError)();
    }
    /**
     *
     * Message
    ...
    async messageContact(messageId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageContact(%s)', messageId);
    ...
    async messageImage(messageId, imageType) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageImage(%s, %s[%s])', messageId, imageType, wechaty_puppet_1.ImageType[imageType]);
        // const attachment = this.mocker.MockMessage.loadAttachment(messageId)
    ...
    async messageRecall(messageId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageRecall(%s)', messageId);
        return false;
    }
    ...
    async messageUrl(messageId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageUrl(%s)', messageId);
        // const attachment = this.mocker.MockMessage.loadAttachment(messageId)
    ...
    async messageMiniProgram(messageId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageMiniProgram(%s)', messageId);
        // const attachment = this.mocker.MockMessage.loadAttachment(messageId)
    ...
    async messageRawPayload(id) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageRawPayload(%s)', id);
        return this.messageStore[id];
    }
    async messageSend(conversationId, something) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageSend(%s, %s)', conversationId, something);
        if (!this.id) {
        ...
        if (!this.whatsapp) {
            wechaty_puppet_1.log.warn('PuppetWhatsApp', 'messageSend() this.client not found');
            return;
        }
    ...
    async messageSendContact(conversationId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageSendUrl(%s, %s)', conversationId, contactId);
    ...
    async messageSendUrl(conversationId, urlLinkPayload) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageSendUrl(%s, %s)', conversationId, JSON.stringify(urlLinkPayload));
    ...
    async messageSendMiniProgram(conversationId, miniProgramPayload) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageSendMiniProgram(%s, %s)', conversationId, JSON.stringify(miniProgramPayload));
    ...
    async messageForward(conversationId, messageId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'messageForward(%s, %s)', conversationId, messageId);
    }
    /**
     *
     * Room
    ...
    async roomRawPayload(id) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomRawPayload(%s)', id);
        return {};
    }
    async roomList() {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomList()');
        return [];
    }
    async roomDel(roomId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomDel(%s, %s)', roomId, contactId);
    }
    async roomAvatar(roomId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomAvatar(%s)', roomId);
        const payload = await this.roomPayload(roomId);
        if (payload.avatar) {
            return wechaty_puppet_1.FileBox.fromUrl(payload.avatar);
        }
        wechaty_puppet_1.log.warn('PuppetWhatsApp', 'roomAvatar() avatar not found, use the chatie default.');
        return (0, config_js_1.qrCodeForChatie)();
    }
    async roomAdd(roomId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomAdd(%s, %s)', roomId, contactId);
    }
    async roomTopic(roomId, topic) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomTopic(%s, %s)', roomId, topic);
    ...
    async roomCreate(contactIdList, topic) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomCreate(%s, %s)', contactIdList, topic);
        return 'mock_room_id';
    }
    async roomQuit(roomId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomQuit(%s)', roomId);
    }
    async roomQRCode(roomId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomQRCode(%s)', roomId);
        return roomId + ' mock qrcode';
    }
    async roomMemberList(roomId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomMemberList(%s)', roomId);
        return [];
    }
    async roomMemberRawPayload(roomId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomMemberRawPayload(%s, %s)', roomId, contactId);
        return {
    ...
    async roomMemberRawPayloadParser(rawPayload) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomMemberRawPayloadParser(%s)', rawPayload);
        return rawPayload;
    }
    ...
    async roomInvitationAccept(roomInvitationId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomInvitationAccept(%s)', roomInvitationId);
    }
    async roomInvitationRawPayload(roomInvitationId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomInvitationRawPayload(%s)', roomInvitationId);
    }
    async roomInvitationRawPayloadParser(rawPayload) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'roomInvitationRawPayloadParser(%s)', JSON.stringify(rawPayload));
        return rawPayload;
    }
    /**
     *
    ...
    async friendshipSearchPhone(phone) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'friendshipSearchPhone(%s)', phone);
        return null;
    }
    async friendshipSearchWeixin(weixin) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'friendshipSearchWeixin(%s)', weixin);
        return null;
    }
    async friendshipAdd(contactId, hello) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'friendshipAdd(%s, %s)', contactId, hello);
    }
    async friendshipAccept(friendshipId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'friendshipAccept(%s)', friendshipId);
    }
    /**
     *
     * Tag
    ...
    async tagContactAdd(tagId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'tagContactAdd(%s)', tagId, contactId);
    }
    async tagContactRemove(tagId, contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'tagContactRemove(%s)', tagId, contactId);
    }
    async tagContactDelete(tagId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'tagContactDelete(%s)', tagId);
    }
    async tagContactList(contactId) {
        wechaty_puppet_1.log.verbose('PuppetWhatsApp', 'tagContactList(%s)', contactId);
        return [];
    }
}
exports.PuppetWhatsapp = PuppetWhatsapp;
PuppetWhatsapp.VERSION = config_js_1.VERSION;
