...
class Io {
    constructor(options) {
        ...
        this.eventBuffer = [];
        this.state = new wechaty_puppet_1.StateSwitch('Io', { log: wechaty_puppet_1.log });
        ...
        wechaty_puppet_1.log.verbose('Io', 'instantiated with apihost[%s], token[%s], protocol[%s], cuid[%s]', options.apihost, options.token, options.protocol, this.id);
    ...
    async start() {
        wechaty_puppet_1.log.verbose('Io', 'start()');
        if (this.lifeTimer) {
        ...
        try {
            ...
            this.lifeTimer = setInterval(() => {
                if (this.ws && this.connected()) {
                    wechaty_puppet_1.log.silly('Io', 'start() setInterval() ws.ping()');
            ...
            wechaty_puppet_1.log.warn('Io', 'start() exception: %s', e.message);
            this.state.off(true);
            throw e;
        }
    }
    initEventHook() {
        wechaty_puppet_1.log.verbose('Io', 'initEventHook()');
    ...
    async initWebSocket() {
        wechaty_puppet_1.log.verbose('Io', 'initWebSocket()');
    ...
    async wsOnOpen(ws) {
        if (this.protocol !== ws.protocol) {
            wechaty_puppet_1.log.error('Io', 'initWebSocket() require protocol[%s] failed', this.protocol);
            // XXX deal with error?
        }
        wechaty_puppet_1.log.verbose('Io', 'initWebSocket() connected with protocol [%s]', ws.protocol);
    ...
    async wsOnMessage(data) {
        wechaty_puppet_1.log.silly('Io', 'initWebSocket() ws.on(message): %s', data);
        ...
        try {
            ...
            wechaty_puppet_1.log.verbose('Io', 'on(message) recv a non IoEvent data[%s]', data);
        }
        switch (ioEvent.name) {
            case 'botie':
                {
                    ...
                    try {
                        if (args[0] === 'message' && args.length === 1) {
                            ...
                            wechaty_puppet_1.log.warn('Io', 'server pushed function is invalid. args: %s', JSON.stringify(args));
                        }
                    }
                    catch (e) {
                        wechaty_puppet_1.log.warn('Io', 'server pushed function exception: %s', e);
                        this.options.wechaty.emit('error', e);
                    }
                }
                break;
            case 'reset':
                wechaty_puppet_1.log.verbose('Io', 'on(reset): %s', ioEvent.payload);
                await this.options.wechaty.reset(ioEvent.payload);
                break;
            case 'shutdown':
                wechaty_puppet_1.log.info('Io', 'on(shutdown): %s', ioEvent.payload);
                process.exit(0);
                // eslint-disable-next-line
                break;
            case 'update':
                wechaty_puppet_1.log.verbose('Io', 'on(update): %s', ioEvent.payload);
                {
                    const wechaty = this.options.wechaty;
                    if (wechaty.logonoff()) {
                        const loginEvent = {
            ...
            case 'sys':
                ...
                break;
            case 'logout':
                wechaty_puppet_1.log.info('Io', 'on(logout): %s', ioEvent.payload);
                await this.options.wechaty.logout();
                break;
            case 'jsonrpc':
                wechaty_puppet_1.log.info('Io', 'on(jsonrpc): %s', ioEvent.payload);
                try {
                    const request = ioEvent.payload;
                    if (!(0, io_peer_js_1.isJsonRpcRequest)(request)) {
                        wechaty_puppet_1.log.warn('Io', 'on(jsonrpc) payload is not a jsonrpc request: %s', JSON.stringify(request));
                        return;
                    }
                    if (!this.jsonRpc) {
                    ...
                    if (!response) {
                        wechaty_puppet_1.log.warn('Io', 'on(jsonrpc) response is undefined.');
                        return;
                    }
                    const payload = json_rpc_peer_1.default.parse(response);
                    const jsonrpcEvent = {
                        name: 'jsonrpc',
                        payload,
                    };
                    wechaty_puppet_1.log.verbose('Io', 'on(jsonrpc) send(%s)', response);
                    await this.send(jsonrpcEvent);
                }
                catch (e) {
                    wechaty_puppet_1.log.error('Io', 'on(jsonrpc): %s', e);
                }
                break;
            default:
                wechaty_puppet_1.log.warn('Io', 'UNKNOWN on(%s): %s', ioEvent.name, ioEvent.payload);
                break;
        }
    }
    ...
    wsOnError(e) {
        wechaty_puppet_1.log.warn('Io', 'initWebSocket() error event[%s]', e && e.message);
        if (!e) {
            return;
        }
        this.options.wechaty.emit('error', e);
    ...
    wsOnClose(ws, code, message) {
        if (this.state.on()) {
            wechaty_puppet_1.log.warn('Io', 'initWebSocket() close event[%d: %s]', code, message);
            ws.close();
            this.reconnect();
        }
    }
    reconnect() {
        wechaty_puppet_1.log.verbose('Io', 'reconnect()');
        if (this.state.off()) {
            wechaty_puppet_1.log.warn('Io', 'reconnect() canceled because state.target() === offline');
            return;
        }
        if (this.connected()) {
            wechaty_puppet_1.log.warn('Io', 'reconnect() on a already connected io');
            return;
        }
        if (this.reconnectTimer) {
            wechaty_puppet_1.log.warn('Io', 'reconnect() on a already re-connecting io');
            return;
        }
        ...
        wechaty_puppet_1.log.warn('Io', 'reconnect() will reconnect after %d s', Math.floor(this.reconnectTimeout / 1000));
    ...
    async send(ioEvent) {
        ...
        const ws = this.ws;
        if (ioEvent) {
            wechaty_puppet_1.log.silly('Io', 'send(%s)', JSON.stringify(ioEvent));
            this.eventBuffer.push(ioEvent);
        }
        else {
            wechaty_puppet_1.log.silly('Io', 'send()');
        }
        if (!this.connected()) {
            wechaty_puppet_1.log.verbose('Io', 'send() without a connected websocket, eventBuffer.length = %d', this.eventBuffer.length);
            return;
        }
        const list = [];
        ...
        try {
            ...
            wechaty_puppet_1.log.error('Io', 'send() exception: %s', e.stack);
            throw e;
        }
    }
    async stop() {
        wechaty_puppet_1.log.verbose('Io', 'stop()');
        if (!this.ws) {
            throw new Error('no ws');
        }
     ...
     */
    async ioMessage(m) {
        wechaty_puppet_1.log.silly('Io', 'ioMessage() is a nop function before be overwritten from cloud');
        if (typeof this.onMessage === 'function') {
            await this.onMessage(m);
        }
    }
    async syncMessage(m) {
        wechaty_puppet_1.log.silly('Io', 'syncMessage(%s)', m);
        const messageEvent = {
            name: 'message',
            payload: m.payload,
        };
