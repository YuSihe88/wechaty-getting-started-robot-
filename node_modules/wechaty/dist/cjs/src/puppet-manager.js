...
class PuppetManager {
    static async resolve(options) {
        wechaty_puppet_1.log.verbose('PuppetManager', 'resolve({puppet: %s, puppetOptions: %s})', options.puppet, JSON.stringify(options.puppetOptions));
        /**
         ...
         */
        if ((0, wechaty_puppet_1.looseInstanceOfPuppet)(options.puppet)) {
    ...
    static async resolveName(puppetName) {
        wechaty_puppet_1.log.verbose('PuppetManager', 'resolveName(%s)', puppetName);
        if (!puppetName) {
            throw new Error('must provide a puppet name');
        }
        if (!(puppetName in puppet_config_js_1.PUPPET_DEPENDENCIES)) {
            throw new Error([
                '',
    ...
    static async installAll() {
        wechaty_puppet_1.log.info('PuppetManager', 'installAll() please wait ...');
        const skipList = [
        ...
        wechaty_puppet_1.log.info('PuppetManager', `installAll() Shell: "npm install ${args.join(' ')}"\n`);
        const result = child_process_1.default.spawn('npm', args, { stdio: 'inherit' });
