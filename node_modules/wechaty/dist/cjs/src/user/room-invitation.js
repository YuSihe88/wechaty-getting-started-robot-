...
class RoomInvitation {
    ...
    constructor(id) {
        this.id = id;
        wechaty_puppet_1.log.verbose('RoomInvitation', 'constructor(id=%s)', id);
        guard_wechatify_class_js_1.guardWechatifyClass.call(this, RoomInvitation);
    }
     ...
     */
    async accept() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'accept()');
        await this.wechaty.puppet.roomInvitationAccept(this.id);
        const inviter = await this.inviter();
        const topic = await this.topic();
        try {
            await inviter.ready();
            wechaty_puppet_1.log.verbose('RoomInvitation', 'accept() with room(%s) & inviter(%s) ready()', topic, inviter);
            return;
        }
        catch (e) {
            wechaty_puppet_1.log.warn('RoomInvitation', 'accept() inviter(%s) is not ready because of %s', inviter, (e && e.message) || e);
        }
    }
    /**
     ...
     */
    async inviter() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'inviter()');
        const payload = await this.wechaty.puppet.roomInvitationPayload(this.id);
    ...
    async memberCount() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'memberCount()');
        const payload = await this.wechaty.puppet.roomInvitationPayload(this.id);
     ...
     */
    async memberList() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'roomMemberList()');
        const payload = await this.wechaty.puppet.roomInvitationPayload(this.id);
     ...
     */
    async date() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'date()');
        const payload = await this.wechaty.puppet.roomInvitationPayload(this.id);
    ...
    static async fromJSON(payload) {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'fromJSON(%s)', typeof payload === 'string'
            ? payload
     ...
     */
    async toJSON() {
        wechaty_puppet_1.log.verbose('RoomInvitation', 'toJSON()');
        const payload = await this.wechaty.puppet.roomInvitationPayload(this.id);
...
function wechatifyRoomInvitation(wechaty) {
    wechaty_puppet_1.log.verbose('RoomInvitation', 'wechatifyRoomInvitation(%s)', wechaty);
