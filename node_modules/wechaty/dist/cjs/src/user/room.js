...
const guard_qr_code_value_js_1 = require("../helper-functions/pure/guard-qr-code-value.js");
...
const room_events_js_1 = require("../events/room-events.js");
 ...
 *
 */
class Room extends room_events_js_1.RoomEventEmitter {
    /**
     * @hideconstructor
    ...
    constructor(id) {
         ...
         *
         */
        _Room_payload.set(this, void 0);
        wechaty_puppet_1.log.silly('Room', `constructor(${id})`);
        guard_wechatify_class_js_1.guardWechatifyClass.call(this, Room);
    }
    ...
    static async create(contactList, topic) {
        wechaty_puppet_1.log.verbose('Room', 'create(%s, %s)', contactList.join(','), topic);
        if (!contactList
        ...
        try {
            ...
            wechaty_puppet_1.log.error('Room', 'create() exception: %s', (e && e.stack) || e.message || e);
    ...
    static async findAll(query) {
        wechaty_puppet_1.log.verbose('Room', 'findAll(%s)', JSON.stringify(query) || '');
        try {
            ...
            while (batchIndex * BATCH_SIZE < roomList.length) {
                ...
                await Promise.all(batchRoomList.map(room => room.ready()
                    .catch(e => {
                    wechaty_puppet_1.log.warn('Room', 'findAll() room.ready() rejection:\n%s', e.stack);
            ...
            wechaty_puppet_1.log.verbose('Room', 'findAll() rejected: %s', e.message);
            console.error(e);
            (0, raven_js_1.captureException)(e);
            return []; // fail safe
        }
    ...
    static async find(query) {
        wechaty_puppet_1.log.verbose('Room', 'find(%s)', JSON.stringify(query));
        ...
        if (roomList.length > 1) {
            wechaty_puppet_1.log.warn('Room', 'find() got more than one(%d) result', roomList.length);
        }
        let n = 0;
        for (n = 0; n < roomList.length; n++) {
            ...
            if (valid) {
                wechaty_puppet_1.log.verbose('Room', 'find() confirm room[#%d] with id=%d is valid result, return it.', n, room.id);
                return room;
            }
            else {
                wechaty_puppet_1.log.verbose('Room', 'find() confirm room[#%d] with id=%d is INVALID result, try next', n, room.id);
            }
        }
        wechaty_puppet_1.log.warn('Room', 'find() got %d rooms but no one is valid.', roomList.length);
        return null;
    }
    /**
     * @ignore
     ...
     */
    toString() {
        if (!__classPrivateFieldGet(this, _Room_payload, "f")) {
            return this.constructor.name;
        }
        return `Room<${__classPrivateFieldGet(this, _Room_payload, "f").topic || 'loading...'}>`;
    }
    async *[(_Room_payload = new WeakMap(), Symbol.asyncIterator)]() {
    ...
    async ready(forceSync = false) {
        wechaty_puppet_1.log.verbose('Room', 'ready()');
        ...
        __classPrivateFieldSet(this, _Room_payload, await this.wechaty.puppet.roomPayload(this.id), "f");
        if (!__classPrivateFieldGet(this, _Room_payload, "f")) {
        ...
        while (batchIndex * BATCH_SIZE < memberList.length) {
            ...
            await Promise.all(batchMemberList.map(c => c.ready().catch(e => wechaty_puppet_1.log.error('Room', 'ready() member.ready() exception:\n%s', e.stack))));
            batchIndex++;
        }
    }
    /**
     ...
     */
    isReady() {
        return !!(__classPrivateFieldGet(this, _Room_payload, "f"));
    }
    ...
    async say(something, ...varList) {
        wechaty_puppet_1.log.verbose('Room', 'say(%s, %s)', something, varList.join(', '));
        let text;
        let msgId;
        if (something instanceof message_js_1.Message) {
        ...
        if (typeof something === 'string') {
            ...
            let mentionList = [];
            if (varList.length > 0) {
                const allIsContact = varList.every(c => c instanceof contact_js_1.Contact);
                if (!allIsContact) {
    ...
    async sayTemplateStringsArray(textList, ...varList) {
        const mentionList = varList.filter(v => v instanceof contact_js_1.Contact);
        // const receiver = {
        //   contactId : (mentionList.length && mentionList[0].id) || undefined,
        //   roomId    : this.id,
        // }
        if (varList.length === 0) {
            ...
            for (; i < varListLength; i++) {
                if (varList[i] instanceof contact_js_1.Contact) {
     ...
     */
    async add(contact) {
        wechaty_puppet_1.log.verbose('Room', 'add(%s)', contact);
    ...
    async remove(contact) {
        wechaty_puppet_1.log.verbose('Room', 'del(%s)', contact);
    ...
    async del(contact) {
        wechaty_puppet_1.log.verbose('Room', 'del(%s)', contact);
        wechaty_puppet_1.log.warn('Room', 'del() is DEPRECATED, use remove() instead.');
     ...
     */
    async quit() {
        wechaty_puppet_1.log.verbose('Room', 'quit() %s', this);
    ...
    async topic(newTopic) {
        wechaty_puppet_1.log.verbose('Room', 'topic(%s)', newTopic || '');
        if (!this.isReady()) {
            wechaty_puppet_1.log.warn('Room', 'topic() room not ready');
            throw new Error('not ready');
        }
        if (typeof newTopic === 'undefined') {
            if (__classPrivateFieldGet(this, _Room_payload, "f") && __classPrivateFieldGet(this, _Room_payload, "f").topic) {
                return __classPrivateFieldGet(this, _Room_payload, "f").topic;
            }
            else {
                const memberIdList = await this.wechaty.puppet.roomMemberList(this.id);
        ...
        const future = this.wechaty.puppet
            ...
            .catch(e => {
            wechaty_puppet_1.log.warn('Room', 'topic(newTopic=%s) exception: %s', newTopic, (e && e.message) || e);
            (0, raven_js_1.captureException)(e);
        });
        return future;
    }
     ...
     */
    async announce(text) {
        wechaty_puppet_1.log.verbose('Room', 'announce(%s)', typeof text === 'undefined'
            ? ''
            : `"${text || ''}"`);
     ...
     */
    async qrCode() {
        wechaty_puppet_1.log.verbose('Room', 'qrCode()');
    ...
    async readMark(hasRead) {
        try {
            ...
            wechaty_puppet_1.log.error('Room', 'readMark() exception: %s', e.message);
        }
    }
    /**
    ...
    async memberAll(query) {
        wechaty_puppet_1.log.silly('Room', 'memberAll(%s)', JSON.stringify(query) || '');
        if (!query) {
    ...
    async member(queryArg) {
        wechaty_puppet_1.log.verbose('Room', 'member(%s)', JSON.stringify(queryArg));
        let memberList;
        // ISSUE #622
        ...
        if (memberList.length > 1) {
            wechaty_puppet_1.log.warn('Room', 'member(%s) get %d contacts, use the first one by default', JSON.stringify(queryArg), memberList.length);
        }
        return memberList[0];
    }
    /**
     ...
     */
    async memberList() {
        wechaty_puppet_1.log.verbose('Room', 'memberList()');
        const memberIdList = await this.wechaty.puppet.roomMemberList(this.id);
        if (!memberIdList) {
            wechaty_puppet_1.log.warn('Room', 'memberList() not ready');
            return [];
        }
     ...
     */
    owner() {
        wechaty_puppet_1.log.verbose('Room', 'owner()');
        const ownerId = __classPrivateFieldGet(this, _Room_payload, "f") && __classPrivateFieldGet(this, _Room_payload, "f").ownerId;
        if (!ownerId) {
            return null;
        }
     ...
     */
    async avatar() {
        wechaty_puppet_1.log.verbose('Room', 'avatar()');
        return this.wechaty.puppet.roomAvatar(this.id);
    }
}
exports.Room = Room;
function wechatifyRoom(wechaty) {
    wechaty_puppet_1.log.verbose('Room', 'wechatifyRoom(%s)', wechaty);
