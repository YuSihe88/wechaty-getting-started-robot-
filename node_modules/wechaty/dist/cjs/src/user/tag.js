...
class Tag {
    ...
    constructor(id) {
        ...
        guard_wechatify_class_js_1.guardWechatifyClass.call(this, Tag);
    }
    ...
    static load(id) {
        if (!this.pool) {
            wechaty_puppet_1.log.verbose('Tag', 'load(%s) init pool', id);
            this.pool = new Map();
        }
        if (this === Tag) {
    ...
    static async delete(tag, target) {
        ...
        try {
             ...
             */
            if (!target || target === contact_js_1.Contact || target === this.wechaty.Contact) {
    ...
    async add(to) {
        ...
        try {
            if (to instanceof contact_js_1.Contact) {
                await this.wechaty.puppet.tagContactAdd(this.id, to.id);
            }
            else if (to instanceof favorite_js_1.Favorite) {
    ...
    async remove(from) {
        ...
        try {
            if (from instanceof contact_js_1.Contact) {
                await this.wechaty.puppet.tagContactRemove(this.id, from.id);
            }
            else if (from instanceof favorite_js_1.Favorite) {
...
function wechatifyTag(wechaty) {
    wechaty_puppet_1.log.verbose('Tag', 'wechatifyTag(%s)', wechaty);
