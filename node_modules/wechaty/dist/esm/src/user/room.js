 ...
 *
 */
import { log, looseInstanceOfFileBox, PayloadType, } from 'wechaty-puppet';
import { FOUR_PER_EM_SPACE, } from '../config.js';
import { captureException } from '../raven.js';
import { guardQrCodeValue, } from '../helper-functions/pure/guard-qr-code-value.js';
...
class Room extends RoomEventEmitter {
    ...
    static async find(query) {
        ...
        for (n = 0; n < roomList.length; n++) {
            ...
            if (valid) {
                log.verbose('Room', 'find() confirm room[#%d] with id=%d is valid result, return it.', n, room.id);
                return room;
            }
            else {
                log.verbose('Room', 'find() confirm room[#%d] with id=%d is INVALID result, try next', n, room.id);
            }
        }
    ...
    async ready(forceSync = false) {
        ...
        while (batchIndex * BATCH_SIZE < memberList.length) {
            ...
            await Promise.all(batchMemberList.map(c => c.ready().catch(e => log.error('Room', 'ready() member.ready() exception:\n%s', e.stack))));
            batchIndex++;
        }
    }
    /**
    ...
    async say(something, ...varList) {
        log.verbose('Room', 'say(%s, %s)', something, varList.join(', '));
        let text;
        let msgId;
    ...
    async sayTemplateStringsArray(textList, ...varList) {
        const mentionList = varList.filter(v => v instanceof Contact);
        // const receiver = {
        //   contactId : (mentionList.length && mentionList[0].id) || undefined,
        //   roomId    : this.id,
        // }
